...
export async function GET() {
  try {
    ...
    const formattedCounties = locationData.map(county => ({
      id: county.id,
      name: county.name,
      subcounties: county.subcounties.map(sub => ({
        name: sub.name,
        towns: sub.towns // no need to wrap towns in another object, just store the array of strings
      }))
    }));

    const inserted = await County.insertMany(formattedCounties);

    return Response.json({ message: '✅ Location data seeded', inserted });
  } catch (error) {
    console.error('Seeding error:', error);
    return Response.json({ message: '❌ Seeding failed', error: error.message });
  }
}
